		...
		setPager = function (pgid, tp){
			...
			$('input.ui-pg-input',"#"+pgcnt).keypress( function(e) {
				...
				if(key === 13) {
					...
					if(($(this).val()>0)){
						ts.p.page=$(this).val()>=ts.p.lastpage ? ts.p.lastpage:ts.p.page;
					}else{
						ts.p.page=1;
					}
...
i=function(e,h,k,aR){
	return e.each(function(){
		...
		h.bind('mousedown',{e:e,k:k},function(v){
			console.info("p.left", p.left);
			console.info("f('left')", f('left'));
			var d=v.data,p={};E=d.e;E1 = aR ? $(aR) : false;
			// attempt utilization of dimensions plugin to fix IE issues
			if(E.css('position') != 'relative'){try{p=E.position();}catch(e){}}
			M={
				X:p.left||f('left')||0,
				Y:p.top||f('top')||0,
