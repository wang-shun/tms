...
public class HttpsRequest implements IServiceRequest {
	...
	private void init() throws IOException, KeyStoreException,
		...
		FileInputStream instream = new FileInputStream(new File(
				Configure.getInstance().getCertLocalPath()));// 加载本地的证书进行https加密传输
		try {
			keyStore.load(instream, Configure.getInstance().getCertPassword().toCharArray());// 设置证书密码
		...
		SSLContext sslcontext = SSLContexts
				...
				.loadKeyMaterial(keyStore,
						Configure.getInstance().getCertPassword().toCharArray()).build();
